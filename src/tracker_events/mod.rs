...
use rerun::{
    components::{Point2D, Radius, Scalar, TextEntry},
    ...
    for unit_tag in updated_units {
        if let Some(unit) = sc2_rerun.sc2_state.units.get(&unit_tag) {
            ...
            MsgSender::new(format!("Unit/{}/Init", unit_init.unit_tag_index))
                .with_splat(Point2D::new(unit_pos.x(), unit_pos.y()))?
...
pub fn register_unit_died(
    ...
    MsgSender::new(format!(
        "Death/{}", // TODO: unit_tags are re-used so we need to find a way to make them unique
        unit_tag(unit_dead.unit_tag_index, unit_dead.unit_tag_recycle),
    ))
    .with_splat(Point2D::new(unit_dead.x as f32, -1. * unit_dead.y as f32))?
...
pub fn add_tracker_event(
    ...
    match &evt {
        ReplayTrackerEvent::UnitInit(unit_init) => {
            register_unit_init(sc2_rerun, unit_init, updated_units, recording_stream)?;
        }
